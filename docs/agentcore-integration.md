...
- `src/client/lib/agents.ts` — the agent roster (`AGENTS`) and starter suggestions. Four
  agents ship, **all `wired: true`**: `general`, `dev`, `order`, `req_plan`.
  ...
  `dev` / `req_plan` use `/api/chat` → IAM `InvokeAgentRuntime`.
