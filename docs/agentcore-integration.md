...
| `RUNTIME_ARN_REQ_DEV` | parked | — | Runtime ARN for Request Developer (`InvokeAgentRuntime`) |
...
- `src/client/lib/agents.ts` — the agent roster (`AGENTS`) and starter suggestions. Two
  ...
  Request Developer / Planner are parked (see comments in `agents.ts` + `RUNTIME_ENV_BY_AGENT`).
